...
PARAM = [
    ...
    '-w', '1640',               # image width   1920x1080-16:9-30fps-FOV:partial    (param 8)
    '-h', '1232',               # image height  1640x1232-4:3-40fps-FOV:Full        (param 10)
    '-rot', '180',              # bit rate, for 1080p30 15Mbits/s                   (param 12)
    '-fps', '30',               # frames per second                                 (param 14)
    '-n'                        # No Preview
]
...
if width > 0 and width <= 3280: PARAM[8] = str(width)
# video height
height = setup_file.getParam("height")
if height > 0 and height <= 2464: PARAM[10] = str(height)
# frames per second
fps = setup_file.getParam("fps")
if fps > 0 and fps <= 90: PARAM[14] = str(fps)
# camera rotation
rot = setup_file.getParam("rotation")
if rot > 0 and rot <= 360: PARAM[12] = str(rot)
